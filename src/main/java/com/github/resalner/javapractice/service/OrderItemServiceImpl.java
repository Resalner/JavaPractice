...
public class OrderItemServiceImpl {
  ...
  public OrderItem getOrderItem(long id){
    return orderItemRepository.findById(id).orElseThrow(()->EntityNotFoundException("не найден элемент заказа с id = "+id));
  }

  public void deleteOrderItem(long id){
    orderItemRepository.deleteById(id).orElseThrow(()->EntityNotFoundException("не найден элемент заказа с id = "+id));
  }
  public OrderItem updateOrderItem(long id, OrderItemRequest orderItemRequest){
    OrderItem or = orderItemRepository.findById(id).orElseThrow(()->EntityNotFoundException("не найден элемент заказа с id = "+id));
